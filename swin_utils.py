...
class inception_mixer(nn.Module):
    def __init__(self,embd_dim=144):
        super(inception_mixer, self).__init__()
        self.embd = embd_dim
        self.high = nn.Sequential(nn.MaxPool1d(3,1,1),
                                  Mlp(embd_dim,embd_dim*4,embd_dim))
        self.pred = Mlp(embd_dim*2,embd_dim,embd_dim)

    def forward(self,x):
        # x : [B N D]
        B,N,D = x.shape
        x_h = self.high(x)
        x = self.pred(torch.cat([x_h,x],dim=2))
        return x
...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=1, in_chans=1, embed_dim=96, norm_layer=None):
        super().__init__()
